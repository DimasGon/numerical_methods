...
def threepoint_approximation__second_order(u, k, h, t, **kwargs):
    ...
    if 'i' in kwargs:
        if kwargs['i'] == 0:
            return list(map(
                lambda j: round(j, 10)), [ 2*t_2 + h_2 + 4*sqrt(h)*t_2 + 5*h_2*t_2, -2*t_2, 0, 2*h_2*u[k-1, 0] - h_2*u[k-2, 0] ]
            ))
        elif kwargs['i'] == N:
            return list(map(
                lambda j: round(j, 10)), [ 2*t_2 + h_2 - 4*sqrt(h)*t_2 + 5*h_2*t_2, -2*t_2, 0, 2*h_2*u[k-1, N] - h_2*u[k-2, N] ]
            ))
    else:
...
def get_error(split_x, split_t, u, N, last_layer):
    ...
    for k, t in enumerate(split_t):
        ...
        for i, x in enumerate(split_x):
            true_res[i] = U(x, t)
        error[k] = linalg.norm(true_res - u[k])
        ...
        #     print(u[k][N-1])
        #     print('\n')
        #     plt.plot(split_x, true_res, color='green')
        #     plt.plot(split_x, u[k], color='red')
        #     plt.grid(True)
        #     plt.title('График')
        #     plt.show()
...
def solve(method, approximation, second_initial_condition, t_end, num_split_x, sigma):
    ...
    num_split_t = ceil(t_end / (h * sigma))
    split_t = linspace(t_start, t_end, num_split_t)
    t = split_t[1] - split_t[0]
    # print(len(split_x), num_split_x)
    # print(len(split_t), num_split_t)
    # print(h, t)
    ...
    plt.plot(split_x, u[last_layer-1], color='red')
    plt.grid(True)
    plt.title('График')
...
if __name__ == '__main__':
    solve('Явный', 'Двухточечная 2-ого порядка', '1-ого порядка', 5, 50, 0.1)